...
// Global variables
let cashBox = document.getElementById("cash"); // changed (cash) to ("cash")
let billBox = document.getElementById("bill"); // changed (bill) to ("bill")
let changeBox = document.getElementById("change"); // changed (change) to ("change")

// Event handlers to be run when the cash or bill value changes
cashBox.addEventListener("change", runTheRegister); // changed runRegister to function name runTheRegister
billBox.addEventListener("change", runTheRegister); // changed runRegister to function name runTheRegister
...
function runTheRegister() {
   ...
   try {
      if (changeValue < 0) {  //Check if the changeValue is not greater than or equal to zero
      ...
      document.getElementById("warning").innerHTML = error.message; // Display error message
   }
}
...
function calcChange(changeValue) {
   ...
   changeValue -=  bill5Amt*5;  // changed bill5Amt*3 to bill5Amt*5
